...
const AdventuresPage: React.FC = () => {
  ...
  const filteredArcs = useMemo(() => {
    return adventureArcs.filter(arc => {
      ...
      // The interest filter can be adapted later if we have categories for arcs
      // For now, it is unused but the UI is kept.
        ...
        <section className="content-wrapper mb-20 px-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredArcs.map((arc) => (
              <Link to={`/adventure-detail?arcId=${arc.id}`} key={arc.id}>
                <motion.div
